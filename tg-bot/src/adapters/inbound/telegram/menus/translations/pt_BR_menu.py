...
class PtBrMenu(BaseMenu):
    def start(self) -> types.BotCommand:
        return types.BotCommand(
            command="/start",
            description=self._localization_service.translate("cmd_start", lang="pt-BR"),
        )

    def menu(self) -> types.BotCommand:
        return types.BotCommand(
            command="/menu",
            description=self._localization_service.translate("cmd_menu", lang="pt-BR"),
        )

    def help(self) -> types.BotCommand:
        return types.BotCommand(
            command="/help",
            description=self._localization_service.translate("cmd_help", lang="pt-BR"),
        )
