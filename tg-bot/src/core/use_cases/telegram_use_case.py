...
class TelegramTaskCallbackUseCase(TelegramTaskCallbackInputPort):
    def __init__(self, bot: Bot, localization_service: LocalizationService) -> None:
        self._bot = bot
    ...
    async def process_result(self, task_result: TaskResult) -> None:
        ...
        if task_result.status == TaskStatus.FAILURE:
            error_msg = self._localization_service.translate(
        ...
        match task_result.task_type:
            case TaskType.RANDOM_NUMBER:
                ...
                message_text = self._localization_service.translate(
            ...
            case TaskType.UNKNOWN | _:
                unknown_msg = self._localization_service.translate(
